...
import { getAuth, getFirestore, getMessaging } from "./lib/admin"
import { CreateOrderSchema, OrderStatusSchema } from "./lib/schemas"
import { computeOrderPricing, type ProductDoc } from "./lib/pricing"
import { nextOrderNumber } from "./lib/orderNumber"
import { enforceRateLimit, getClientIp, hashIp } from "./lib/rateLimit"
...
export const createOrder = onRequest(async (req, res) => {
  try {
    ...
    await enforceRateLimit({ db, ipHash, windowSeconds: 60, maxInWindow: 6 })

    const settingsSnap = await db.doc("settings/global").get()
    const packagingFee = (settingsSnap.data()?.packagingFee as number | undefined) ?? 0
    const deliveryFee = 0
    ...
    const productsById: Record<string, ProductDoc> = {}
    for (const snap of productSnaps) {
      if (!snap.exists) continue
      productsById[snap.id] = snap.data() as ProductDoc
    }

    const { items, pricing } = computeOrderPricing({
      input,
      productsById,
      deliveryFee,
      packagingFee,
...
export const getOrderByToken = onRequest(async (req, res) => {
  try {
    if (req.method !== "GET") {
      res.status(405).json({ error: "METHOD_NOT_ALLOWED" })
      return
...
export const setOrderStatus = onCall(async (request) => {
  ...
  const payload = request.data as { orderId?: string; status?: string }
  const orderId = String(payload.orderId || "")
  const status = OrderStatusSchema.parse(payload.status)
...
export const setupFirstAdmin = onCall(async (request) => {
  ...
  if (hasAdmin) {
    throw new HttpsError("already-exists", "Admin already exists. Use setAdminClaim instead.")
  }
...
export const setAdminClaim = onCall(async (request) => {
  ...
  const payload = request.data as { uid?: string; email?: string }
  const auth = getAuth()
  ...
  if (!targetUid) {
    throw new HttpsError("invalid-argument", "MISSING_UID_OR_EMAIL")
  }
...
export const registerAdminToken = onCall(async (request) => {
  const uid = request.auth?.uid
  const isAdmin = request.auth?.token?.admin === true
  if (!uid || !isAdmin) {
    throw new HttpsError("permission-denied", "ADMIN_REQUIRED")
  ...
  const payload = request.data as { fcmToken?: string; userAgent?: string }
  const fcmToken = String(payload.fcmToken || "")
  if (!fcmToken) {
    throw new HttpsError("invalid-argument", "MISSING_FCM_TOKEN")
  }

  const db = getFirestore()
  const tokenId = crypto.createHash("sha256").update(`${uid}:${fcmToken}`).digest("hex")
...
export const createOrderCallable = onCall(async (request) => {
  const input = CreateOrderSchema.parse(request.data)
  if (input.fulfillment === "DELIVERY" && !input.customer.address) {
    throw new HttpsError("invalid-argument", "ADDRESS_REQUIRED")
  }
  if (input.fulfillment === "PICKUP") {
    input.customer.address = null
  }
  ...
  try {
    ...
    const tokens = tokensSnap.docs.map((d) => d.data()?.fcmToken).filter(Boolean) as string[]
...
export const getOrderByTokenCallable = onCall(async (request) => {
  const payload = request.data as { orderId?: string; token?: string }
  const orderId = String(payload.orderId || "")
...
export const seedProducts = onCall(async () => {
  const db = getFirestore()
